...
- **Classpath-only, `META-INF/services` discovery, with no `provides` clauses.** Works everywhere, including the starter, but forfeits JPMS encapsulation for module-path consumers (internals become reachable API) — the framework instead carries both registrations per provider so neither consumption model is penalised.
