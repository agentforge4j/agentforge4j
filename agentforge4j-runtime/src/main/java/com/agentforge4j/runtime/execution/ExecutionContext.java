 ...
 * executed (which changes when a nested workflow is entered), and a stack of workflow ids for cycle
 * detection.
 *
...
public final class ExecutionContext {
  ...
  /**
   * Parallel stack of workflow definitions (innermost at the head), used for nested workflow
  ...
  /**
   * Monotonic counter for step execution UIDs within this context (one {@code drive} / outer
   * execution). Not shared across executor instances or runs.
   */
  ...
  public void exitWorkflow() {
    Validate.isTrue(!workflowStack.isEmpty(),
        "Cannot exit workflow: stack empty for run '" + state.getRunId() + "'");
    workflowStack.pop();
