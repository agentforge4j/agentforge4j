...
/**
 * Minimal loopback HTTP/1.1 server for deterministic Gemini integration tests (no extra test
 * dependencies).
 */
final class GeminiLoopbackHttpServer implements AutoCloseable {
  ...
  GeminiLoopbackHttpServer(int statusCode, String responseBody) {
  ...
  /**
   * @param responseDelayMillis sleep this long before sending the response (after reading the
   *                            request)
   */
  GeminiLoopbackHttpServer(int statusCode, String responseBody,
      AtomicReference<String> capturedRequestBodyUtf8,
    ...
    this.thread = new Thread(this::serveOnce, "gemini-loopback-http");
