...
/**
 * A human-in-the-loop workflow: an agent step gated by a {@code HUMAN_APPROVAL} transition. The
 * agent does its work and emits {@code COMPLETE}, then the run suspends at
 * {@code AWAITING_STEP_APPROVAL} until a person decides. Approving advances the run to
 * {@code COMPLETED}; rejecting fails it with a {@code StepRejectionFailure}.
 *
 * <p>The run is deterministic and offline. The {@code agentforge4j-llm-fake} provider serves one
 * scripted {@code COMPLETE} for the agent's call, so no real model, network, or API key is
 * involved; the approve/reject decisions are supplied directly in code.
 */
public final class HumanApprovalExample {
  ...
  /**
   * The scripted model output for the agent's single call. An LLM response is a bare JSON array of
   * command objects; a lone {@code COMPLETE} finishes the step, after which the approval gate
   * suspends the run.
   */
  ...
  public static void main(String[] args) throws URISyntaxException {
    ...
    System.out.println("  failure reason: %s".formatted(rejected.getFailureReason()));
  }

  /**
   * Assembles an AgentForge4j runtime wired to the deterministic fake provider and to this module's
   * own workflow and agent configuration. Shared by {@link #main(String[])} and the test so both
  ...
  private static void printStatus(AgentForge4j agentForge4j, String label, String runId) {
    ...
    System.out.println("%s: %s".formatted(label, state.getStatus()));
  }

  /**
   * Resolves a directory on the classpath (e.g. {@code /workflows}) to a filesystem path. Works
   * when the example runs from exploded {@code target/classes} (IDE, tests, {@code mvn} build),
   * which is how examples are run from source.
   *
