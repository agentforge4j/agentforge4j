 ...
 * (or any other secret slot) holds a <em>reference</em> key, and this resolver turns that key into
 * the live value when the integration is invoked. The OSS default
 * ({@link EnvironmentSecretResolver}) reads process environment variables and system properties; an
 * embedding application (for example the platform) supplies its own implementation backed by its
 * secret store.
 *
 * <p>Resolution is deliberately lazy — performed per invocation, not at wiring time — so an unused
 * capability whose secret is absent does not block startup. A reference that cannot be resolved is a
 * fail-fast error. Implementations must never include a resolved value in any log, exception
 * message, or other output.
 */
