...
/**
 * OSS default {@link SecretResolver}: resolves a reference key against the process environment
 * first and JVM system properties second, so a reference such as {@code "GITHUB_TOKEN"} is read
 * from {@link System#getenv(String)} or, failing that, {@link System#getProperty(String)}. A
 * reference with no value in either source is a fail-fast error naming the reference key — never the
 * value.
 *
