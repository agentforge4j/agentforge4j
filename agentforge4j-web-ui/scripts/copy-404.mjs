...
// one artifact. Runs as part of `npm run build`, after `vite build`; fails the build
// (non-zero exit) if the two files are not byte-identical, so this is a build-time
// regression guard on the mechanism, not a separately-run test that can go stale.
