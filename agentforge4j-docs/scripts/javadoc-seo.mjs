 ...
 * carries a robots tag is returned untouched rather than gaining a second, so a double pass can
 * never emit contradictory directives.
 *
 * @param {string} html
 * @returns {string}
 */
...
//
// Anchored on the literal `AgentForge4j API` brand text inside the trailing parenthesised group, not
// on "a trailing parenthesised group" in general: a Javadoc page whose own title legitimately ends
// in parentheses must never have that silently eaten. The optional inner group absorbs the nested
// `(next)` while still matching a plain `AgentForge4j API` window title, so a future pom that drops
// the `(next)` suffix keeps working with no change here.
const WINDOW_TITLE_SUFFIX_PATTERN = /^(.*?)\s*\(AgentForge4j API(?:\s*\([^()]*\))?\)$/;

// The same generated doc title rendered as a visible heading — matched only when the `<h1>`'s
// ENTIRE text is that brand doc title, so a real page heading is never rewritten. Anchored on the
// same `AgentForge4j API` brand text as `WINDOW_TITLE_SUFFIX_PATTERN`, for the same reason.
const DOC_TITLE_HEADING_PATTERN = /(<h1[^>]*>)AgentForge4j API(?:\s*\([^()]*\))?(<\/h1>)/;
  ...
  for (const surface of surfaces) {
    ...
    for (const pageInput of walkHtmlFiles(surfaceRoot)) {
      ...
      if (isJavadocRedirectStub(html)) {
        ...
        if (surface.noindex) {
          const stubHtml = addRobotsNoindexTag(html);
          if (stubHtml !== html) {
