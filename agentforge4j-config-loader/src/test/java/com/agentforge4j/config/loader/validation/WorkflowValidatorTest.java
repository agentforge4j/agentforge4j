...
class WorkflowValidatorTest {
  ...
  void validateBlueprintRefs_rejectsASelfReferentialBlueprintInsteadOfOverflowingTheStack() {
    ...
    // The shared WorkflowTreeWalker guard reports a self-referential blueprint as a depth-limit
    // breach naming the enclosing workflow, rather than naming the repeated blueprint id; what
    // matters here is that it is rejected with a clear error instead of overflowing the stack.
    assertThatThrownBy(() -> validator.validateBlueprintRefs(Map.of("wf1", wf)))
        .isInstanceOf(IllegalArgumentException.class)
